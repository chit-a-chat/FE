...
import { useMatchRecommendation } from "../model/store";
import { MatchRecommendationCard } from "./MatchRecommendationCard/MatchRecommendationCard";

export const MatchRecommendationList = () => {
    const { getMatchRecommendation } = useMatchRecommendation();
    const { data: matchData } = useQuery({
        queryKey: ["test"],
        queryFn: getMatchRecommendation,
        initialData: { matches: [] },
    ...
    return (
        <>
            <FlexDiv
                ...
                ref={scrollElement}
            >
                {matchData.matches.map((aMatch) => (
                    <MatchRecommendationCard key={`match-${aMatch.id}`} match={aMatch} />
                ))}
            </FlexDiv>
            <div
                css={{
